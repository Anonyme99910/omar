...
class SaleController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Ultra-smart search: invoice number, customer name, or phone (optimized)
        if ($request->has('search') && !empty($request->search)) {
            $searchTerm = trim($request->search);
            
            // Remove common phone number formatting characters for better matching
            $cleanPhone = preg_replace('/[\s\-\(\)]+/', '', $searchTerm);
            
            // Check if search term looks like a phone number (contains only digits)
            $isPhoneSearch = preg_match('/^\d+$/', $cleanPhone);
            
            if ($isPhoneSearch) {
                // For phone search: find matching customers first, then filter sales
                $matchingCustomerIds = \App\Models\Customer::all()->filter(function($customer) use ($cleanPhone) {
                    $decryptedPhone = $customer->phone;
                    return $decryptedPhone && strpos($decryptedPhone, $cleanPhone) !== false;
                })->pluck('id')->toArray();
                
                if (!empty($matchingCustomerIds)) {
                    $query->whereIn('customer_id', $matchingCustomerIds);
                } else {
                    // No matching customers, check invoice number
                    $query->where(function($q) use ($searchTerm) {
                        $q->where('invoice_number', $searchTerm)
                          ->orWhere('invoice_number', 'LIKE', "%{$searchTerm}%");
                    });
                }
            } else {
                // Regular search (invoice number or customer name)
                $query->where(function($q) use ($searchTerm) {
                    // 1. Exact invoice number match (highest priority)
                    $q->where('invoice_number', $searchTerm)
                      ...
                      ->orWhereHas('customer', function($customerQuery) use ($searchTerm) {
                          $customerQuery->where(function($cq) use ($searchTerm) {
                              // Exact name match
                              $cq->where('name', $searchTerm)
                                 // Partial name match (start of name)
                                 ->orWhere('name', 'LIKE', "{$searchTerm}%")
                                 // Partial name match (any position)
                                 ->orWhere('name', 'LIKE', "%{$searchTerm}%");
                          });
                      });
                });
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            foreach ($request->items as $item) {
                $productTotal = $item['quantity'] * $item['unit_price'];
                $packagingTotal = 0;
                
                // Add packaging cost if applicable
                if (!empty($item['with_packaging']) && !empty($item['packaging_quantity']) && !empty($item['packaging_price'])) {
                    $packagingTotal = $item['packaging_quantity'] * $item['packaging_price'];
                }
                
                $subtotal += $productTotal + $packagingTotal;
            }
            ...
            foreach ($request->items as $item) {
                ...
                // Add packaging to total if applicable
                if (!empty($item['with_packaging']) && !empty($item['packaging_quantity']) && !empty($item['packaging_price'])) {
                    $totalPrice += $item['packaging_quantity'] * $item['packaging_price'];
                }
