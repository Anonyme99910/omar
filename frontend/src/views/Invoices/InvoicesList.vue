    ...
    <div class="filters">
      <div class="search-wrapper">
        <input 
          ...
          type="text" 
          placeholder="بحث ذكي: رقم الفاتورة، اسم العميل، أو رقم الهاتف..."
          class="search-input"
          @input="handleSearch"
        >
        ...
        <div v-if="filters.search && !isSearching" class="search-results-count">
          {{ invoices.length }} نتيجة
        </div>
      </div>
...
const handleSearch = () => {
  ...
  // Debounce for 200ms (faster response)
...
const loadInvoices = async (page = 1) => {
  try {
    ...
    // Always clear searching indicator
